...
//                        }
    ...
    //标题栏右侧购物车图标按钮，默认显示
    public void setRightShopCartButton(boolean visible, OnClickListener onRightShopCartClickListener) {
        if (visible) {
            rightImageShopCartButton.setVisibility(VISIBLE);
            if (onRightShopCartClickListener != null) {
                rightImageShopCartButton.setVisibility(VISIBLE);
                rightImageShopCartButton.setOnClickListener(onRightShopCartClickListener);
            }
        } else {
            rightImageShopCartButton.setVisibility(GONE);
        }
    }

    public void setRightShopCartButton(boolean visible) {
        if (visible) {
            rightImageShopCartButton.setVisibility(VISIBLE);
        } else {
            rightImageShopCartButton.setVisibility(GONE);
    ...
    //标题栏右侧购物车上的黑色计数图标按钮，默认隐藏
    public void setShopCartCountertext(String countTxt) {
        if ((!(TextUtils.isEmpty(countTxt))) && (!("0".equals(countTxt))) && (LoginInfo.isUserLogin())) {
            rightImageShopCartCounterButton.setVisibility(VISIBLE);
            rightImageShopCartCounterButton.setText(countTxt);
        } else {
            rightImageShopCartCounterButton.setVisibility(GONE);
        }
    ...
    //标题栏右侧分享图标按钮，默认隐藏
    public void setRightShareButton(OnClickListener listener) {
        if (listener != null) {
            rightImageShareButton.setVisibility(VISIBLE);
            rightImageShareButton.setOnClickListener(listener);
        } else {
            rightImageShareButton.setVisibility(GONE);
        }
    }

    //标题栏右侧设置图标按钮，默认隐藏
    public void setRightSetButton(OnClickListener onRightSetButtonClickListener) {
        if (onRightSetButtonClickListener != null) {
            rightImageSetButton.setVisibility(VISIBLE);
            rightImageSetButton.setOnClickListener(onRightSetButtonClickListener);
        }
    }
