...
public class ChargeBackResultActivity extends BaseActivity {
    ...
    private String[] textStatus = {"退款成功", "商家已拒绝，请与客服联系。"};
    private int[] iconStatus = {R.mipmap.icon_chargeback_result_success, R.mipmap.icon_chargeback_result_fail};
    private ChargeBackListBean.RowsBean chargeBackResultDetailsBean;

    public ChargeBackResultActivity() {
        super(R.layout.activity_chargeback_result);
    }
...
//        requestResultInfo();
    ...
    private void refreshView() {
        ...
        //productInfo
        ChargeBackListBean.RowsBean.ProductBean product = chargeBackResultDetailsBean.getProduct();
        ...
        //stage
        int stage = chargeBackResultDetailsBean.getStage();
        switch (stage) { // 进度：0.取消；1.进行中；2.已完成；3.拒绝;
            case 1:
                textViewCreateTime.setText("申请时间");
