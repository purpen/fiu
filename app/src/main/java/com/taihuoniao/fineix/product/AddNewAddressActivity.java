...
//                Toast.makeText(AddNewAddressActivity.this, "省:" + provinceList.get(provinceView.getCurrentItem()).getName() + ",市:" + provinceList.get(provinceView.getCurrentItem()).getCityList().get(cityView.getCurrentItem()).getName(), Toast.LENGTH_SHORT).show();
        ...
        initProvinceDatas();
        provinceView.setAdapter(new ArrayWheelAdapter<>(provinceNames));
...
//        StatusBarChange.initWindow(AddNewAddressActivity.this);
        titleLayout = (MyGlobalTitleLayout) findViewById(R.id.activity_addnewaddress_title);
...
//                }
    ...
    private void commitAddress() {
        ...
        if (nameEdt.getText().toString().isEmpty()) {
            Toast.makeText(AddNewAddressActivity.this, R.string.name_is_empty, Toast.LENGTH_SHORT).show();
            return;
        }
        if (phoneEdt.getText().toString().isEmpty()) {
            Toast.makeText(AddNewAddressActivity.this, R.string.phone_is_empty, Toast.LENGTH_SHORT).show();
            return;
        }
        if (detailsAddressEdt.getText().toString().isEmpty()) {
            Toast.makeText(AddNewAddressActivity.this, R.string.address_details_is_empty, Toast.LENGTH_SHORT).show();
            return;
        }
        if (postcodeEdt.getText().toString().isEmpty()) {
            Toast.makeText(AddNewAddressActivity.this, R.string.zip_is_empty, Toast.LENGTH_SHORT).show();
            return;
        }
        cityId = provinceList.get(provinceView.getCurrentItem()).getCityList().get(cityView.getCurrentItem()).get_id();
        provinceId = provinceList.get(provinceView.getCurrentItem()).get_id();
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case DataConstants.DELETE_ADDRESS:
                    ...
                    if (success) {
                        Toast.makeText(AddNewAddressActivity.this, R.string.delete_success, Toast.LENGTH_SHORT).show();
...
//                case 8:
//                    dialog.dismiss();
////                    provinceList=THNApplication.provinceList;
//                    provinceList = THNApplication.provinceList;
//                    initPopupWindow();
//                    break;
//                case 8:
...
//                    break;
    ...
    private void putData() {
        ...
        if (addressBean != null) {
            titleLayout.setRightButton("删除", new View.OnClickListener() {
                @Override
                public void onClick(View v) {
...
//                    if (provinceList.get(i).get_id().equals(addressBean.getProvince())) {
